...
public class SixthLabFinish {
    public static String encryptWithSecondKey(String[] message, int frequency, String symbols) {
        String[] encrypted_message = new String[message.length + message.length / (frequency - 1)];
        String[] symbols_repeated = StringUtils.repeat(symbols, encrypted_message.length).split("");
        int messageIterator = 0;
        int symbolsIterator = 0;
        for (int i = 0; i < encrypted_message.length; i++) {
            symbolsIterator++;
            if (symbolsIterator % frequency == 0) {
                encrypted_message[i] = symbols_repeated[messageIterator];
                symbolsIterator = 0;
                messageIterator++;
            } else {
    ...
    public static void main(String[] args) {
        ...
        while (!message.equalsIgnoreCase("Стоп")) {
            if (message.equalsIgnoreCase("Шифруем")) {
                ...
                System.out.println("Введите символы для пустых ячеек:");
                String secondKeySymbols = scanner.nextLine();
                System.out.println("Введите частоту повторения пустого символа:");
                int repeatFrequency = Integer.parseInt(scanner.nextLine());
                String unencryptedMessageString = encryptWithSecondKey(preUnencryptedMessageString, repeatFrequency, secondKeySymbols);
                System.out.println(unencryptedMessageString);
